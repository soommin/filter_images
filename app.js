...
// DOM 요소들 변수 저장
const btns = document.querySelectorAll(".filter-btns button"); // 버튼 요소를 저장
const images = document.querySelectorAll(".filter-image"); // 이미지 요소를 저장
...
// 라이트박스 활성화 함수 = 클릭한 이미지를 thisElmt 파라미터로 전달
const activateLightBox = (thisElmt) => {
  ...
  // 라이트 박스가 활성화 될 때 클릭된 이미지 요소의 src 속성 값을 ㅇ릭어 라이트 박스 이미지 src 속성 값으로 전달
...
// 이미지 요소 배열을 순회한다. 첫번째 파라미터는 이미지 요소 배열, 두번째 파라미터는 boolean 값 - ture일 경우 ? 뒤에 : 을 기준으로 앞에 적용되고, false일 경우 뒤에 로직이 적용 된다.
...
// methpd : 어떠한 객체의 멤버가 함수일 경우 메서드라고 부른다.
// Array.from() : 유사배열(Nodeist, HTMLCollection)을 정식 배열로 전환
